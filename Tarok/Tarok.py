...
def main_on_press(key):
    ...
    with lock:
        ...
        if key == keyboard.KeyCode.from_char('v'):
            State = "WAVE"
            print("V key pressed - Changing state to WAVE")
...
# Wave state function
def Wave_State(bus0, bus1, bus2, bus3, ID_1, ID_2, ID_3):
    '''
    State function for Wave gait.
    This function computes a Wave gait trajectory and runs it a single cycle before returning to standing pose.
    The State includes a transfer phase from standing pose to an offset position and back to standing pose at the end of the state.

    Press "v" to switch to wave state

    Note: It is not possible to exit this state using ctrl+c - It completes the full cycle and then returns to STAND state
    ...
    Returns:
    - None, but runs the Wave trajectory a single cycle, before it returns to standing pose
    '''
    print("Executing WAVE state")
    print("Performing pre-computations for WAVE state...")
    ...
    ### OFFSET -> WAVE -> Stand ###   
    print("Pre-computations complete, executing Wave state...")
    print("Moving to offset position...")

    # Writing walk PI parameters for transition from stand to offset position and entire wave trajectory
    PID_RAM_Control(bus0,ID_1, pi_bezier_walk)
    PID_RAM_Control(bus0,ID_2, pi_bezier_walk)
    PID_RAM_Control(bus0,ID_3, pi_bezier_walk)
    PID_RAM_Control(bus1,ID_1, pi_bezier_walk)
    ...
    time.sleep(0.5) # Sleep for a short time to ensure transition to walk start position is complete before starting loop, adjust as needed
    ...
    time.sleep(0.5) # Sleep for a short time to ensure wave motion is complete before starting next transition, adjust as needed

    print("Moving back to stand position...")
    ...
    while True:
        ...
        if current_state == "UP/DOWN":
            ...
            with lock:
                State = "STAND" # After finishing the state function, switch back to standing pose state
            returned_from_state = True
        elif current_state == "WAVE":
            Wave_State(bus0, bus1, bus2, bus3, ID_1, ID_2, ID_3)
            with lock:
                State = "STAND" # After finishing the state function, switch back to standing pose state
            returned_from_state = True
