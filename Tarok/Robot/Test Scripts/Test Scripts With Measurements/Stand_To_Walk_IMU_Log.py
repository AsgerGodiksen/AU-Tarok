# Script to perform stand to walk transistion and walk for x cycles while logging IMU data
# Used as proof of concept for the IMU data logging and to see how the IMU data looks during walking gait
...
log_dir = os.path.join(SCRIPT_DIR, "TEST_DATA_IMU_Stand_To_Walk")
os.makedirs(log_dir, exist_ok=True)
timestamp_str = time.strftime('%Y-%m-%d_%H-%M-%S')
log_filename = os.path.join(log_dir, f"STW_IMU_Log_{test_number}_{timestamp_str}.csv")
...
# Preallocate: timestamp + index  + pitch + roll = 4 columns
data = np.zeros((500000, 4))
data_count = 0
...
try:
    while True:
        ...
        # Ensure all feedback is received before proceeding to log data
        fb_FL = f_FL.result()
        fb_FR = f_FR.result()
        fb_HL = f_HL.result()
        fb_HR = f_HR.result()
        ...
        # Log IMU orientation data
        data[data_count, :] = [elapsed_total, index, pitch, roll]
        data_count += 1
