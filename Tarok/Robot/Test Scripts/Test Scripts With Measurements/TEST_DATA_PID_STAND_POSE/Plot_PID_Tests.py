...
csv_files = glob.glob(os.path.join(DATA_DIR, "Stand_Pose_Torque_Log_TEST_*.csv"))
...
for test_num in sorted(groups.keys()):
    ...
    if len(files) == 1:
        name = f"Stand_Pose_Torque_log_Test_{test_num:02d}"
        plot_test(files[0], name)
    else:
        # Multiple files for same test number → label a, b, c...
        for idx, path in enumerate(files):
            suffix = chr(ord("a") + idx)
            name = f"Stand_Pose_Torque_log_Test_{test_num:02d}{suffix}"
            plot_test(path, name)
