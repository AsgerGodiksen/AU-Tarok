...
def can_listener(bus):
    # Thread function to continuously listen for CAN messages and update latest_status
    while running:
        for motor_id in [ID_1, ID_2, ID_3]:
            result = Read_Torque_Current(bus, motor_id)
            if result is None:
                continue
            Torque_Current, Torque = result
            with lock:
...
log_filename = os.path.join(log_dir, f"Stand_Pose_Torque_Log_{time.strftime('%Y-%m-%d_%H-%M-%S')}.csv")
