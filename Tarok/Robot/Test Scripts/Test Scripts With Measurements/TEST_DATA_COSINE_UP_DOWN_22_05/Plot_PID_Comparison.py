...
for leg, ax in leg_axes1.items():
    for j, (jname, jcol) in enumerate(zip(JOINTS, JOINT_COLORS)):
        ax.plot(t_ref_plot, analytical_plot[leg][:, j],
                color=jcol, linestyle="--", linewidth=1.4, alpha=0.75, zorder=2)
    for k, (num, df) in enumerate(tests.items()):
        ...
        for j, (jname, jcol) in enumerate(zip(JOINTS, JOINT_COLORS)):
            ax.plot(t_meas, df[f"{leg}_{jname}_Pos (deg)"].values,
                    color=jcol, linestyle=mstyle, linewidth=1.5, zorder=3)
...
fig1.legend(
    ...
    fontsize=LEGEND_SIZE, framealpha=0.9, bbox_to_anchor=(0.5, -0.03),
)
fig1.tight_layout()
...
fig2.legend(
    ...
    fontsize=LEGEND_SIZE, framealpha=0.9, bbox_to_anchor=(0.5, -0.0),
)
fig2.tight_layout()
...
# ══════════════════════════════════════════════════════════════════════════════
# 5.  Summary statistics + report file
# ══════════════════════════════════════════════════════════════════════════════
import datetime
...
for num, df in tests.items():
    ...
    rms_sums[num] = rms_sum
emit("─" * 60)
emit(f"\n{'Sum of all 12 RMS errors per test':}")
for num, rms_sum in rms_sums.items():
    emit(f"  Test {num}: {rms_sum:.3f} deg")
emit("─" * 60)
...
for num, df in tests.items():
    ...
    with open(pid_path, "a") as f:
        ...
        f.write("─" * 52 + "\n")
        f.write(f"Sum of all 12 RMS errors: {rms_sums[num]:.3f} deg\n")
        f.write("─" * 52 + "\n")
    print(f"Statistics appended to {os.path.basename(pid_path)}")

timestamp = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
test_tag  = '_vs_'.join(str(n) for n in tests)

fig1_path = os.path.join(DATA_DIR, f"PID_Comparison_Fig1_MeasVsRef_{test_tag}_{timestamp}.png")
