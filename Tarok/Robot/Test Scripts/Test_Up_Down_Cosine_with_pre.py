...
# pre and post trajectory time series
pre_time = (total_time / 4) * 1.5
post_time = (total_time / 4) * 1.5
num_time_steps_pre = int(pre_time / dt) + 1
num_time_steps_post = int(post_time / dt) + 1
t_pre = np.linspace(0, 1.5, num_time_steps_pre)
t_post = np.linspace(0, 1.5, num_time_steps_post)

# Define desired end-effector trajectory for pre and post trajectory
x_FL_pre = x_FR_pre = (l_k/2)*np.ones_like(t_pre)  # X position in meters (constant)
x_HL_pre = x_HR_pre = (-l_k/2)*np.ones_like(t_pre)  # X position in meters (constant)
y_FL_pre = y_HL_pre = (w_k/2 + 0.078)*np.ones_like(t_pre)  # Y position in meters (constant)
y_FR_pre = y_HR_pre = (-w_k/2 - 0.078)*np.ones_like(t_pre)  # Y position in meters (constant)
z_pre = cos_interp(t_pre, -0.41, -0.36, 0, 1.5) # cosine wave from -0.41 to -0.36 over 1.5 seconds
x_FL_post = x_FR_post = (l_k/2)*np.ones_like(t_post)  # X position in meters (constant)
x_HL_post = x_HR_post = (-l_k/2)*np.ones_like(t_post)  # X position in meters (constant)
y_FL_post = y_HL_post = (w_k/2 + 0.078)*np.ones_like(t_post)  # Y position in meters (constant)
y_FR_post = y_HR_post = (-w_k/2 - 0.078)*np.ones_like(t_post)  # Y position in meters (constant)
z_post = cos_interp(t_post, -0.36, -0.41, 0, 1.5) # cosine wave from -0.36 to -0.41 over 1.5 seconds

# Define desired end-effector velocity for pre and post trajectory
x_dot_pre = np.zeros_like(t_pre)  # X velocity in meters/second (constant)
y_dot_pre = np.zeros_like(t_pre)  # Y velocity in meters/second (constant)
z_dot_pre = cos_interp_dot(t_pre, -0.41, -0.36, 0, 1.5) # derivative of cosine wave from -0.41 to -0.36 over 1.5 seconds
x_dot_post = np.zeros_like(t_post)  # X velocity in meters/second (constant)
y_dot_post = np.zeros_like(t_post)  # Y velocity in meters/second (constant)
z_dot_post = cos_interp_dot(t_post, -0.36, -0.41, 0, 1.5) # derivative of cosine wave from -0.36 to -0.41 over 1.5 seconds

# Segment boundaries for the trajectory: 0->2s, 2->4s
...
### SAMLING - ikke brug sidste fra pre og første fra post for at undgå duplikering af points
...
#### SHOULD BE INTITAL POSITION OF PRE-LOOP Trajectory
