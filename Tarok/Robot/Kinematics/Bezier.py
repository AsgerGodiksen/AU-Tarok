...
def Building_Stand_To_Walk_Trajectories(Swing_Time_Scalar, Swing_Time_Steps, Phase_Offset, FL_walk_start_x, FR_walk_start_x, HL_walk_start_x, HR_walk_start_x):
    ...
    FL_Stand_To_Walk_Velocities_With_Transfer[:, Swing_Start_Index_with_transfer['HL'] + Swing_Time_Steps :] = cos_interp_dot(t_transfer, FL_Stand_To_Walk_With_Transfer[:, Swing_Start_Index_with_transfer['HL'] + Swing_Time_Steps - 1].reshape((3, 1)), FL_Stand_To_Walk[:, -1].reshape((3, 1)), 0, Stand_To_Walk_Transfer_Time)
    ...
    HR_Stand_To_Walk_Velocities_With_Transfer[:, Swing_Start_Index_with_transfer['HL'] + Swing_Time_Steps :] = cos_interp_dot(t_transfer, HR_Stand_To_Walk_With_Transfer[:, Swing_Start_Index_with_transfer['HL'] + Swing_Time_Steps - 1].reshape((3, 1)), HR_Stand_To_Walk[:, -1].reshape((3, 1)), 0, Stand_To_Walk_Transfer_Time)
    ...
    FR_Stand_To_Walk_Velocities_With_Transfer[:, Swing_Start_Index_with_transfer['HL'] + Swing_Time_Steps :] = cos_interp_dot(t_transfer, FR_Stand_To_Walk_With_Transfer[:, Swing_Start_Index_with_transfer['HL'] + Swing_Time_Steps - 1].reshape((3, 1)), FR_Stand_To_Walk[:, -1].reshape((3, 1)), 0, Stand_To_Walk_Transfer_Time)
    ...
    HL_Stand_To_Walk_Velocities_With_Transfer[:, Swing_Start_Index_with_transfer['HL'] + Swing_Time_Steps :] = cos_interp_dot(t_transfer, HL_Stand_To_Walk_With_Transfer[:, Swing_Start_Index_with_transfer['HL'] + Swing_Time_Steps - 1].reshape((3, 1)), HL_Stand_To_Walk[:, -1].reshape((3, 1)), 0, Stand_To_Walk_Transfer_Time)
...
def NEW_Building_Stand_To_Walk_Trajectories(Swing_Time_Scalar, Swing_Time_Steps, Phase_Offset, FL_walk_start_x, FR_walk_start_x, HL_walk_start_x, HR_walk_start_x):
    ...
    # Generate stand trajectories in body frame
    x_FL_stand_frontend = np.zeros(2 * Swing_Time_Steps)
    x_FL_stand_backend = FL_end_x * np.ones(Swing_Time_Steps)
    x_FR_stand_backend = FR_end_x * np.ones(3 * Swing_Time_Steps)
    x_HL_stand_frontend = np.zeros(Swing_Time_Steps)
    x_HL_stand_backend = HL_end_x * np.ones(2 * Swing_Time_Steps)
    x_HR_stand_frontend = np.zeros(3 * Swing_Time_Steps)
    z_stand = 0.44 * np.ones(Swing_Time_Steps)

    # Assemble all trajectories in body frame
    # FL
    x_FL = np.concatenate((x_FL_stand_frontend, Swing_Trajectory_FL[:,0], x_FL_stand_backend))
    y_FL = np.zeros(4 * Swing_Time_Steps)
    z_FL = - np.concatenate((z_stand, z_stand, Swing_Trajectory_FL[:,1], z_stand))
    FL_Stand_To_Walk = np.vstack((x_FL, y_FL, z_FL))
    # FR
    x_FR = np.concatenate((Swing_Trajectory_FR[:,0], x_FR_stand_backend))
    y_FR = np.zeros(4 * Swing_Time_Steps)
    z_FR = - np.concatenate((Swing_Trajectory_FR[:,1], z_stand, z_stand, z_stand))
    FR_Stand_To_Walk = np.vstack((x_FR, y_FR, z_FR))
    # HL
    x_HL = np.concatenate((x_HL_stand_frontend, Swing_Trajectory_HL[:,0], x_HL_stand_backend))
    y_HL = np.zeros(4 * Swing_Time_Steps)
    z_HL = - np.concatenate((z_stand, Swing_Trajectory_HL[:,1], z_stand, z_stand))
    HL_Stand_To_Walk = np.vstack((x_HL, y_HL, z_HL))
    # HR
    x_HR = np.concatenate((x_HR_stand_frontend, Swing_Trajectory_HR[:,0]))
    y_HR = np.zeros(4 * Swing_Time_Steps)
    z_HR = - np.concatenate((z_stand, z_stand, z_stand, Swing_Trajectory_HR[:,1]))
    HR_Stand_To_Walk = np.vstack((x_HR, y_HR, z_HR))
    ...
    # Assemble all velocity arrays
    # FL
    x_dot_FL = np.concatenate((np.zeros(2 * Swing_Time_Steps), Swing_Velocity_FL[:,0], np.zeros(Swing_Time_Steps)))
    y_dot_FL = np.zeros(4 * Swing_Time_Steps)
    z_dot_FL = - np.concatenate((dot_stand, dot_stand, Swing_Velocity_FL[:,1], dot_stand))
    FL_Stand_To_Walk_Velocities = np.vstack((x_dot_FL, y_dot_FL, z_dot_FL))
    # FR
    x_dot_FR = np.concatenate((Swing_Velocity_FR[:,0], np.zeros(3 * Swing_Time_Steps)))
    y_dot_FR = np.zeros(4 * Swing_Time_Steps)
    z_dot_FR = - np.concatenate((Swing_Velocity_FR[:,1], dot_stand, dot_stand, dot_stand))
    FR_Stand_To_Walk_Velocities = np.vstack((x_dot_FR, y_dot_FR, z_dot_FR))
    # HL
    x_dot_HL = np.concatenate((np.zeros(Swing_Time_Steps), Swing_Velocity_HL[:,0], np.zeros(2 * Swing_Time_Steps)))
    y_dot_HL = np.zeros(4 * Swing_Time_Steps)
    z_dot_HL = - np.concatenate((dot_stand, Swing_Velocity_HL[:,1], dot_stand, dot_stand))
    HL_Stand_To_Walk_Velocities = np.vstack((x_dot_HL, y_dot_HL, z_dot_HL))
    # HR
    x_dot_HR = np.concatenate((np.zeros(3 * Swing_Time_Steps), Swing_Velocity_HR[:,0]))
    y_dot_HR = np.zeros(4 * Swing_Time_Steps)
    z_dot_HR = - np.concatenate((dot_stand, dot_stand, dot_stand, Swing_Velocity_HR[:,1]))
    HR_Stand_To_Walk_Velocities = np.vstack((x_dot_HR, y_dot_HR, z_dot_HR))
    ...
    # Swing phase start — first discrete step where each leg enters swing
    SWING_SEQUENCE_Mixed = ['FL', 'HR', 'FR', 'HL']
    #SWING_SEQUENCE_Mixed = ['FR', 'HL', 'FL', 'HR']
